...
export default function DateRangePickerUi({
  ...
  const renderCalendar = () => {
    ...
    for (let i = 0; i < firstDayOfMonth; i++) {
      daysArray.push(
        <div key={`empty-${i}`} className="h-[46px] w-[46px]"></div>,
      );
    }
  ...
  return (
    <div className={`relative ${containerClassName}`} ref={datepickerRef}>
      ...
      <div className="relative flex items-center">
        ...
        <input
          type="text"
          className={`block w-full rounded-lg border border-gray-300 bg-gray-50 p-2.5 ps-10 text-sm text-gray-900 focus:border-blue-500 focus:ring-blue-500 dark:border-gray-600 dark:bg-gray-700 dark:text-white dark:placeholder-gray-400 dark:focus:border-blue-500 dark:focus:ring-blue-500 ${inputClassName}`}
          value={updateInput()}
      ...
      {isOpen && (
        <div
          className={`absolute z-10 mt-2 rounded-xl border border-gray-300 bg-white pt-5 shadow-lg dark:border-gray-600 dark:bg-gray-800 ${calendarClassName}`}
        >
          <div className="flex items-center justify-between px-5">
            <button
              type="button"
              role="button"
              className="rounded-md p-2 text-gray-700 transition hover:bg-gray-100 dark:text-gray-300 dark:hover:bg-gray-700"
              onClick={() =>
                setCurrentDate(
          ...
          <div className="mt-2 grid grid-cols-7 gap-y-0.5 px-5">
            {renderCalendar()}
          </div>
          <div className="mt-2 flex justify-end space-x-2.5 border-t border-gray-300 p-5 dark:border-gray-600">
            <button
              type="button"
              role="button"
              className="inline-flex items-center justify-center rounded-lg border border-gray-200 bg-white px-4 py-2 text-center text-sm font-medium text-gray-900 hover:bg-gray-100 hover:text-blue-700 focus:z-10 focus:outline-none focus:ring-4 focus:ring-gray-100 dark:border-gray-600 dark:bg-gray-800 dark:text-gray-400 dark:hover:bg-gray-700 dark:hover:text-white dark:focus:ring-gray-700"
            ...
            <button
              type="button"
              role="button"
              className="flex items-center justify-center rounded-lg bg-blue-700 px-4 py-2 text-center text-sm font-medium text-white hover:bg-blue-800 focus:outline-none focus:ring-4 focus:ring-blue-300 dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800"
              onClick={handleApply}
            >
              Apply
            </button>
